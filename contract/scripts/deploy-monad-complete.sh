...
# Configuration
# Get script directory and calculate relative paths
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
CONTRACT_DIR="$(cd "$SCRIPT_DIR/.." && pwd)"
BACKEND_DIR="$(cd "$CONTRACT_DIR/../backend" && pwd)"
FRONTEND_DIR="$(cd "$CONTRACT_DIR/../frontend" && pwd)"
...
MON="0x42cfaB6eb59D5bBB75CaBc7e2213a8dfFB1A2d2b"
...
MON_USD_FEED="0x31491744e2dbf6df7fcf4ac0820d18a609b49076d45066d3568424e62f686cd1"
...
print_info "Configuring MON price feed (MON/USD)..."
TX_HASH=$(cast send $PYTH_ORACLE \
  "setPriceFeed(address,bytes32)" \
  $MON \
  $MON_USD_FEED \
...
if [ -z "$TX_HASH" ]; then
  print_error "Failed to set MON price feed"
  exit 1
fi
print_success "MON price feed configured (tx: ${TX_HASH:0:10}...)"
...
print_info "Testing MON price fetch..."
MON_PRICE=$(cast call $PYTH_ORACLE "getPrice(address)(uint256)" $MON --rpc-url $RPC_URL 2>&1)
if [ $? -eq 0 ]; then
  print_success "MON price: $MON_PRICE"
else
  print_warning "MON price fetch failed (may need Pyth update)"
fi
...
Check MON price:
  cast call $PYTH_ORACLE "getPrice(address)(uint256)" $MON --rpc-url $RPC_URL
